...
def get_node_classification_tgb_data(dataset_name: str):
    ...
    for node_label_time in tqdm(label_dict.keys()):
        for src_node_id in label_dict[node_label_time].keys():
            ...
            converted_label_dict[(node_label_time, src_node_id)
                                 ] = label_dict[node_label_time][src_node_id]
    ...
    if os.path.exists(f"./saved_labeled_node_interaction_indices/{dataset_name}.npy"):
        ...
        labeled_node_interaction_indices = np.load(
            f"./saved_labeled_node_interaction_indices/{dataset_name}.npy", allow_pickle=True).tolist()
    else:
        # dictionary, key is a tuple (interact time, node id), value is the interaction index
        ...
        for (node_label_time, src_node_id) in tqdm(converted_label_dict.keys(), desc=f"data preprocessing for {dataset_name}"):
            ...
            nodes_historical_interactions_mask = (src_node_ids == src_node_id) & (
                node_interact_times <= node_label_time)
            if len(edge_ids[nodes_historical_interactions_mask]) > 0:
                ...
                print(
                    "Warning: a labeled node is not matched, use the first interaction to match it")
            ...
            labeled_node_interaction_indices[(
                node_label_time, src_node_id)] = nodes_most_recent_interaction_idx
        assert len(converted_label_dict.keys()) == len(
            labeled_node_interaction_indices.keys()), "Mismatched dictionary keys!"

        os.makedirs(f"./saved_labeled_node_interaction_indices", exist_ok=True)
        np.save(
            f"./saved_labeled_node_interaction_indices/{dataset_name}.npy", labeled_node_interaction_indices)
    ...
    assert min_val_time > node_interact_times[train_mask].max(
    ), "Train data and validation data are mixed!"
    assert min_test_time > node_interact_times[val_mask].max(
    ), "Validation data and test data are mixed!"

    for (node_label_time, src_node_id) in tqdm(converted_label_dict.keys()):
        interaction_idx = labeled_node_interaction_indices[(
            node_label_time, src_node_id)]
        labels[interaction_idx] = converted_label_dict[(
            node_label_time, src_node_id)]
